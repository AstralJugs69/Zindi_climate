...
def cv_bag_component(train: pd.DataFrame, test: pd.DataFrame, mode: str) -> np.ndarray:
    ...
    test_dummy[TARGET] = 0
    xt = add_interactions(make_view(test_dummy, "demographics_time"), mode)
    xt = xt[x.columns]
...
def main():
    ...
    report_dir = Path("reports/interaction_candidates")
    report_dir.mkdir(parents=True, exist_ok=True)

    components = {}
    for mode in ("categorical", "numeric", "all"):
        print(f"CV-bagging interaction component: {mode}_ctr1 (15 fold models) ...", flush=True)
        components[mode] = cv_bag_component(train, test, mode)

    candidates = {
        "v201_interactions_cat75_num25.csv": (
            0.75 * components["categorical"] + 0.25 * components["numeric"]
        ),
        "v202_interactions_cat50_all50.csv": (
            0.50 * components["categorical"] + 0.50 * components["all"]
        ),
        "v203_interactions_all_ctr1.csv": components["all"],
    }
    ...
    pd.DataFrame(
        {
            ...
            "categorical_ctr1": components["categorical"],
            "numeric_ctr1": components["numeric"],
            "all_ctr1": components["all"],
        }
    ...
    metadata = {
        "validation_reference": {
            "v201": {
                "blend": "0.75 categorical_ctr1 + 0.25 numeric_ctr1",
                "oof_f1": 0.818660,
                "oof_auc": 0.825011,
                "oof_score": 0.821201,
            },
            "v202": {
                "blend": "0.50 categorical_ctr1 + 0.50 all_ctr1",
                "oof_f1": 0.817287,
                "oof_auc": 0.825792,
                "oof_score": 0.820689,
            },
